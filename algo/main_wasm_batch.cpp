...
extern "C" {
    ...
    // TRUE BATCH PROCESSING: Process multiple puzzles in a single WASM call
    int solveBatch(int count) {
        ...
        // Serial processing instead of parallel (no OpenMP)
        for (int i = 0; i < count; i++) {
            SudokuSolver solver;
            solver.initialize(inputBuffer + (i * 81));
            bool solved = solver.solve();

            solvedFlags[i] = solved;
            if (solved) {
                memcpy(outputBuffer + (i * 81), solver.getSolution(), 81);
                solvedCount++;
            } else {
                // For unsolved puzzles, copy original puzzle
                memcpy(outputBuffer + (i * 81), inputBuffer + (i * 81), 81);
            }
        }
